...
function perkalianUnik(arr) {
    var jml = []; //tambahkan variabel baru(bebas)
    for (var i = 0; i < arr.length; i++) { //perulangan menggunakan for
        var tetap = 1; //tambah variabel baru
        for (var j = 0; j < arr.length; j++) { //perulangan for (kedua)
            if (j !== i) { //kondisi jika nilai J tidak sama dengan i
                //rumus
                tetap *= arr[j];
            }
        }
